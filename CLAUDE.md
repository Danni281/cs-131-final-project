...
| 7 | Real-time optimization | done | `--depth-downsample` (default 2) rasterizes depth at shape/2 then upsamples; `--process-downsample` (default 1; =2 doubles live FPS but softens output). 720p bench: ds1/ps1 42.7ms, ds2/ps1 38.4ms, ds2/ps2 24.1ms = 41.5 FPS warp-only. Live pipeline ~30 FPS at default, ~34 FPS at ds2/ps2. |
| 8 | Evaluation script | done | `src/eval.py`: 5 variants (raw / per_frame / ema / kalman / oneeuro) on a recorded clip; reports per-stage latency, FPS, pre/post nose ratio, frame-to-frame jitter; outputs JSON + two-panel plot. Headline: `results/eval_default.png`. |
