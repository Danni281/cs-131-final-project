...
def main() -> None:
    ...
    gaps_closed = []
    for raw_m, corr_m in zip(summary["raw_mean"], summary["corr_mean"]):
        gap = raw_m - gt
        if abs(gap) < 1e-6:
            gaps_closed.append(0.0)
        else:
            moved = raw_m - corr_m
            gaps_closed.append(float(moved / gap))
    ...
    ax1.errorbar(x, summary["corr_mean"], yerr=summary["corr_std"],
                 marker="s", capsize=3, label=f"dense corrected (alpha={args.alpha})",
    ...
    ax2.set_ylabel("% of raw->GT gap closed by correction")
